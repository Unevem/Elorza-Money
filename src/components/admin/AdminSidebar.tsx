...
export function AdminSidebar({ currentOrg, onOrgChange }: Props) {
  ...
  return (
    <aside className="w-60 h-full bg-white border-r border-gray-200 flex flex-col shadow-sm flex-shrink-0">
      {/* Brand */}
      <div className="flex flex-col gap-6 flex-1 overflow-y-auto">
        ...
        <nav className="flex flex-col gap-1 px-4">
          ...
          <Link href="/admin" className={`flex items-center gap-3 px-3 py-2 rounded-lg font-semibold text-sm transition-colors ${pathname === '/admin' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
            <LayoutDashboard size={15} /> Dashboard
          </Link>
          <Link href="/admin/transacoes" className={`flex items-center gap-3 px-3 py-2 rounded-lg font-semibold text-sm transition-colors ${pathname === '/admin/transacoes' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
            <Receipt size={15} /> Transações
          </Link>
          <Link href="/admin/categories" className={`flex items-center gap-3 px-3 py-2 rounded-lg font-semibold text-sm transition-colors ${pathname === '/admin/categories' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      {/* Footer */}
      <div className="px-4 pb-5 flex flex-col gap-1 border-t border-gray-100 pt-4 mt-auto">
